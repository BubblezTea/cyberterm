...
CRITICAL RULES:
0. **COMBAT TRIGGER** – Only include the "combat" field when the player explicitly attempts to engage in physical violence (e.g., "I punch", "I attack", "I shoot", "I swing my weapon"). Do NOT trigger combat for social intimidation, fleeing, or simply encountering a hostile NPC. Use the "roll" field for intimidation, persuasion, escape attempts, or any non‑violent action instead.
...
9. **VARIETY IN QUESTS AND NPC INTERACTIONS** – Avoid repetitive quest structures. Do NOT default to "retrieve a package from Oni-Kiru", "meet at the old clock tower on 5th and Main", or "V gives a generic job". Instead, create unique quests that fit the current location, the player's class and backstory, and the NPC's personality. Use a wide range of objectives: sabotage, extraction, negotiation, data theft, assassination, smuggling, protection, etc. Locations should be diverse: different districts, buildings, underground facilities, nightclubs, rooftops, etc. The NPC "V" should not always give a package retrieval job; vary their role and requests.
...
Schema:
{
  "narration": "string — immersive second-person narration, 1-4 sentences",
  ...
  async send(userMessage, extraContext) {
    ...
    const result = await queueRequest(async () => {
      const raw = await callProvider(messages, MAX_TOKENS);
