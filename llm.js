...
6. SKILLS ARE COMBAT ONLY – Every skill must be a combat action used during battle. No skill may be a passive, a social ability, a hacking tool, a traversal move, or anything that only makes sense outside of combat. Every skill must have a clear in-combat use: deal damage, apply a status effect, restore HP/energy, or debuff the enemy. If a skill has no damage and no status effect, it must restore HP or energy and have a cooldown of at least 3. Cooldowns: powerful skills (high damage, strong status) 5-10, basic attacks 0, utility/medium 1-4. Never exceed 10.
...
Schema:
{
  ...
  parse(raw) {
    ...
    try {
      ...
      const openCurlies = (cleaned.match(/{/g) || []).length;
      const closeCurlies = (cleaned.match(/}/g) || []).length;
      const openSquares = (cleaned.match(/\[/g) || []).length;
      const closeSquares = (cleaned.match(/\]/g) || []).length;

      let repaired = cleaned;
      // close any open string by trimming to last complete value
      repaired = repaired.replace(/,?\s*"[^"]*$/, '');
      repaired += ']'.repeat(Math.max(0, openSquares - closeSquares));
      repaired += '}'.repeat(Math.max(0, openCurlies - closeCurlies));

      try {
        return JSON.parse(repaired);
      } catch(e2) {
        console.warn('JSON repair also failed:', e2.message);
      }
